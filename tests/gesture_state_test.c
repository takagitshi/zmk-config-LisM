...
int main(void) {
    ...
    assert(update(&state, LISM_GESTURE_AXIS_X, 40, false, 1000) == LISM_GESTURE_NONE);
    assert(update(&state, LISM_GESTURE_AXIS_Y, 30, true, 1000) == LISM_GESTURE_RIGHT);

    lism_gesture_state_reset(&state);
    assert(update(&state, LISM_GESTURE_AXIS_X, 59, true, 1000) == LISM_GESTURE_NONE);
    assert(update(&state, LISM_GESTURE_AXIS_X, 1, true, 1015) == LISM_GESTURE_RIGHT);
    assert(state.x == 0 && state.y == 0);

    lism_gesture_state_reset(&state);
    assert(update(&state, LISM_GESTURE_AXIS_X, -60, true, 1000) == LISM_GESTURE_LEFT);

    lism_gesture_state_reset(&state);
    assert(update(&state, LISM_GESTURE_AXIS_X, 30, false, 1000) == LISM_GESTURE_NONE);
    assert(update(&state, LISM_GESTURE_AXIS_Y, 30, true, 1000) == LISM_GESTURE_RIGHT);

    lism_gesture_state_reset(&state);
    assert(update(&state, LISM_GESTURE_AXIS_X, 29, false, 1000) == LISM_GESTURE_NONE);
    assert(update(&state, LISM_GESTURE_AXIS_Y, -31, true, 1000) == LISM_GESTURE_UP);
    assert(state.x == 0 && state.y == 0);

    lism_gesture_state_reset(&state);
    assert(update(&state, LISM_GESTURE_AXIS_X, 60, true, 1000) == LISM_GESTURE_RIGHT);
    assert(update(&state, LISM_GESTURE_AXIS_X, 100, true, 1100) == LISM_GESTURE_NONE);
    assert(state.x == 0 && state.y == 0);
    assert(update(&state, LISM_GESTURE_AXIS_X, 59, true, 1150) == LISM_GESTURE_NONE);
    assert(state.x == 0 && state.y == 0);
    assert(update(&state, LISM_GESTURE_AXIS_X, 59, true, 1165) == LISM_GESTURE_NONE);
    ...
    assert(state.x == 0 && state.y == 0);
    assert(update(&state, LISM_GESTURE_AXIS_X, 60, true, 1165) == LISM_GESTURE_RIGHT);
